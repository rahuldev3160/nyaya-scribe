...
| DECIDE-25 | UPSC Essay Paper module uses adaptive architecture — essay count, year, framework, hook type are all data in DB registries; no hardcoding in code | Annual refresh must be a data operation (scripts), not a code change; adding 2027 essays = 2 script runs, zero code change | Hardcode 20 essays for 2026 in seed file | .knowledge/plans/PLAN-018.md |
| DECIDE-26 | Essay framework: PART A (A1 hook→A4 signpost) / PART B (dimension blocks: CLAIM→EVIDENCE→ANALYSIS→COUNTER→REBUTTAL + CHALLENGES + SOLUTIONS) / PART C (C1 synthesis→C4 closing line) | Provides a replicable formula users can internalize; dimensions stored as JSON array in body_dimensions_json → framework-agnostic (PESTLE, SPIDER, IDEA, PPF all fit same column) | Separate table per framework type | .knowledge/plans/PLAN-018.md; essay_model_answers schema |
| DECIDE-27 | All Essay + Ethics model answers generated in ONE Anthropic Batch API call (315 items = 36 essay + 279 ethics) | 50% batch discount; one call vs. many reduces scheduling overhead; all content arrives together for single seed run | Separate batch per module | scripts/essay/generate_answers.py + scripts/ethics/generate_answers.py |
| DECIDE-28 | GS4 Ethics paper simulation is self-compare only — no AI scoring | Ethics has no objectively correct answer; AI scoring of value judgements would be misleading; confirmed by PLAN-017 design which also disables ai_scoring for paper_id='gs4' | Run AI scoring with a disclaimer | ethics_attempts table has no ai_score_json; no essay_eval gate needed |
| DECIDE-29 | STAKE framework for Ethics case studies (Stakeholders→Tension→Analysis→Key decision→Execution); IDEA-U for theory (Introduce→Dimensions→Evidence→Apply→Upshot) | Research confirmed all 13 years of sub-part questions map cleanly to S/T/A/K/E steps; IDEA-U word target scales by marks (2/5/10/12.5) | PESTLE for case studies (wrong — PESTLE is business strategy, not dilemma resolution) | .knowledge/plans/PLAN-019.md |
| DECIDE-30 | Indexing layer uses normalized junction tables (essay_thinker_links, ethics_concept_links, ethics_thinker_links, ethics_scenario_links) + SQLite FTS5 virtual tables; JSON blobs replaced for M:M relationships | Enables 8+ query dimensions without code changes; FTS5 allows keyword search across question text + preambles + answers | JSON arrays with LIKE queries (not indexable, not composable) | PLAN-018 + PLAN-019 indexing layer sections; m041, m044 migrations |
...
| DECIDE-36 | New law-exam registry follows the existing per-exam pattern — new `law.db` reusing the `pyq_questions`/`model_answers`/`rubrics` schema shape, new blueprint, `g.law_conn` wiring in app.py — with exam_id values for CLC DU semester exams, judiciary services mains (Rajasthan Judicial Service prioritized), and AIBE | Confirmed reusable exam-family pattern in 2026-08-29 Nyaya Arena scoping audit; RJS prioritized over DJS to match Rahul's RAS/Rajasthan focus | New unified schema spanning all exams | Planned: migrations/mXXX_law_db.py, web/blueprints/law_bp.py |
| DECIDE-37 | Existing `_score_answer()` scoring pipeline reused for law answers via system-prompt + rubric swap only — no structural change to the function itself | Confirmed exam-agnostic in structure during the 2026-08-29 audit; only the system prompt string and dimension labels are UPSC-specific today | Build a separate law-specific scoring function | web/blueprints/ies_quiz_bp.py `_score_answer()` (to be parameterized, not duplicated) |
| DECIDE-38 | Any new law-exam PYQ ingestion (judiciary, AIBE, CLC DU past papers) requires mandatory 5–10 sample manual eyeball validation against a known-clean reference before bulk seeding, and must prefer official sources (Bar Council of India, High Court sites, DU Faculty of Law) over scraped third-party/coaching content | BUG-035's root cause was a scraped coaching-site page dumping comment-section junk as "questions" (65–90% contamination in gs1-3) — same failure class applies to any new scrape-based law PYQ pipeline | Trust a single scraped source without sampling | Applies to future scripts/seed_law_pyqs.py (not yet written) |
| DECIDE-39 | Stay on Railway as the live host (already paid, just restored from trial-expiry outage); build a portable Docker + backup readiness package instead of migrating now | Rahul owns a separately-paid hosting domain but the live-cutover risk/effort isn't justified while Railway works; readiness package means a future move is a known quantity, not a from-scratch scramble | Migrate to Rahul's own server now | Dockerfile, .dockerignore, scripts/backup_production_data.sh, docs/PORTABLE_HOSTING.md (commit 6d6ba40); DIAG-004 |
| DECIDE-40 | RBI English timed-writing sim warns on section time-budget overrun rather than hard-cutting into the next section | Rahul's explicit choice (2026-08-30): the training goal is making time cost visible, not exactly replicating the exam's forced cutoff — his actual 2026 failure was not finishing, so awareness-first training was judged more useful than a punitive hard stop | Hard-cut like the real exam | web/blueprints/rbi_english_sim_bp.py; PLAN-021 Area 4 |
...
| SCHEMA-09 | essay_theme_analysis + essay_frameworks + essay_hook_types + essay_quotes — registries for adaptive essay pipeline | upsc_gs.db | m039 | All seeded once; adding new framework = 1 row; adding new hook = 1 row |
| SCHEMA-10 | essay_topic_proposals — annual CA→topic inference queue with human-review status | upsc_gs.db | m040 | status: pending/approved/rejected/needs_edit |
| SCHEMA-11 | essay_questions + essay_ca_links (M:M) + essay_thinker_links (M:M) + SQLite indexes + FTS5 (essay_fts + essay_answer_fts) | upsc_gs.db | m041 | backing_note column shows users why topic was chosen; is_high_probability flag for pre-exam hotflagging |
| SCHEMA-12 | essay_model_answers (PART A/B/C indexed sub-parts + body_dimensions_json) + essay_generation_batches | upsc_gs.db | m042 | body_dimensions_json: [{slot,label,claim,evidence,analysis,counter,rebuttal}] |
| SCHEMA-13 | essay_attempts | upsc_gs.db | m043 | intro_text + body_text + conclusion_text + full_text; no AI scoring for essay module |
| SCHEMA-14 | ethics_questions + ethics_practice_papers + ethics_concept_links + ethics_thinker_links + ethics_scenario_links + ethics_concept_analysis + ethics_scenario_analysis + SQLite indexes + FTS5 | upsc_gs.db | m044 | section: A (theory) / B (case_study); question_type: definition/short/medium/analytical/case_study |
| SCHEMA-15 | ethics_model_answers (IDEA-U fields for Section A + STAKE fields for Section B) + ethics_attempts (self_rating only, no AI score) | upsc_gs.db | m045, m046 | |
...
| PLAN-018 | UPSC Essay Paper module — adaptive CA→essay pipeline, PART A/B/C framework, 36 Qs (16 PYQ + 20 practice), batch generation | Phase 0 ✅ (S43) — Phase 1 next: m039–m043 + seed scripts | .knowledge/plans/PLAN-018.md |
| PLAN-019 | GS4 Ethics paper simulation — IDEA-U (theory) + STAKE (case study), PYQ 2019–2025, 2 practice papers, self-compare only | Phase 0 ✅ (S43) — Phase 1 next: m044–m046 + seed scripts | .knowledge/plans/PLAN-019.md |
...
| RISK-09 | Zero bilingual/Hindi handling anywhere in the stack (no language column, no bilingual prompt/UI logic — confirmed via grep) while CLC DU's actual exam is bilingual (100 marks/3hrs/8 questions, answer 5 of 8 at 20 marks, pass 45/100 — sourced from Rahul's real DU PDF) | HIGH | Required before generating/scoring Hindi-medium content at scale: a rendering/pipeline spike covering Devanagari DB storage, prompt language, and template/PDF rendering — none of this is tested today |
| RISK-10 | No ranking/leaderboard concept exists anywhere in this codebase (confirmed via grep, no false positives beyond "NIRF rankings" content mentions) | MEDIUM | New cross-product leaderboard lives in the separate "Nyaya Arena" project, not inside Descriptive-exams' own DB — Scribe's role is to expose scored results via an internal API only, not to own the leaderboard |
