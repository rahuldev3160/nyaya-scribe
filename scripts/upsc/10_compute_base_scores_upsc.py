"""
Stage 10 (UPSC): Compute topic base priority scores from UPSC PYQ frequency data.
Run: python3 scripts/upsc/10_compute_base_scores_upsc.py
...
def compute_scores(conn: sqlite3.Connection) -> list:
    # Load exam config
    cfg = conn.execute(
...
def upsert_scores(conn: sqlite3.Connection, scores: list) -> None:
    for s in scores:
        conn.execute("""
...
def verify(conn: sqlite3.Connection, scores: list) -> None:
    by_paper = {}
    ...
    print("\n── UPSC Stage 10 Sense Check — Base Priority Scores ──")
    ...
    print(f"\nTotal scored topics : {total}")
    print(f"Top 3 by priority   : {', '.join(s['topic_id'] for s in top3)}")
    assert total >= 16, f"Expected >= 16 scored UPSC topics, got {total}"
    print("\n✓ UPSC Base scores computed")
    print("──────────────────────────────────────────────────────\n")
...
if __name__ == "__main__":
    ...
    conn = get_connection()
    print("Computing UPSC base priority scores...")
